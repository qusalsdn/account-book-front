...
export default function Update({ params }: { params?: { id: string } }) {
  ...
  useEffect(() => {
    axios
      .get(`http://3.34.135.190:3000/breakdown/update/${params?.id}`, {
  ...
  const onSubmit = (formData: { date: string }) => {
    axios
      .post(`http://3.34.135.190:3000/breakdown/update/${params?.id}`, formData, {
  ...
  const onClickDeleteBtn = () => {
    ...
    if (confirm)
      axios
        .delete(`http://3.34.135.190:3000/breakdown/delete/${params?.id}`, {
          headers: { Authorization: `Bearer ${accessToken}` },
        })
        .then((res) => {
          if (res.data.ok) router.replace(`/?year=${year}&month=${month}`);
